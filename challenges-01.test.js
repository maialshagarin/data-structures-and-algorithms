...
const greeting = (word) => {
  // Solution code here...
  let change = word.toUpperCase();
  return change ;
};

const speaker = (message, callback) => {
  // Solution code here...
  let show = callback(message) ;
  return show ;
};
...
let fizzBussResult = [];
  arr.forEach((element)=>{
    if ( element%3 ===0 && element%5===0){
      fizzBussResult.push("Fizz Buzz")
    }else if ( element % 3 === 0){
      fizzBussResult.push("Fizz")
    }else if ( element %5 === 0){
      fizzBussResult.push("Buzz")
    }else {
      fizzBussResult.push(element)
    }
  })
  return fizzBussResult ;
};
